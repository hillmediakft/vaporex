...
var References = function () {
    ...
    var enableDisableButtons = function () {
        ...
        var deleteReferenceSubmit = $('button[name="delete_gyik_submit"]');
        ...
        $('[id*=details_]').on('click', function (e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    var update_date = '';
                    if (data.gyik_update_timestamp != null) {
                        update_date = data.gyik_update_timestamp;
                    }
                    var expiry_date = '';
                    if (data.gyik_expiry_timestamp != null) {
                        expiry_date = data.gyik_expiry_timestamp;
                    }

                    var gyik_status = '';
                    if (data.gyik_status == '0') {
                        gyik_status = 'Inaktív';
                    } else {
                        gyik_status = 'Aktív';
                    }
                    ...
                    content += add_html('Azonosító szám', '#' + data.gyik_id);
                    content += add_html('Megnevezés:', data.gyik_title);
                    content += add_html('Munkaadó:', data.employer_name);
                    content += add_html('Típus:', data.gyik_list_name);
                    content += add_html('Leírás:', data.gyik_description);
                    content += add_html('Feltételek:', data.gyik_conditions);
                    content += add_html('Munkavégzés helye:', munka_helye);
                    content += add_html('Munkaidő:', data.gyik_working_hours);
                    content += add_html('Fizetés:', data.gyik_pay);
                    content += add_html('Lejárati idő:', expiry_date);
                    content += add_html('Létrehozás dátuma:', data.gyik_create_timestamp);
                    content += add_html('Módosítás dátuma:', update_date);
                    content += add_html_last('Státusz:', gyik_status);
                    content += '</dl>';
                    ...
                    bootbox.dialog({
                        ...
                        buttons: {
                            success: {
                                ...
                                callback: function () {
                                    window.location.href = 'admin/gyik/update_gyik/' + data.gyik_id;
                                }
                            },
                            cancel: {
                                label: "vissza",
    ...
    var printTable = function () {
        $('#print_gyik').on('click', function (e) {
            e.preventDefault();
            var divToPrint = document.getElementById("gyik");
            console.log(divToPrint);
//		divToPrint = $('#gyik tr').find('th:last, td:last').remove();
            newWin = window.open("");
...
$(document).ready(function () {
    ...
    References.init(); // init gyik page
});