...
var Products = function () {
    ...
    var productsTable = function () {
        ...
        var tableWrapper = jQuery('#products_wrapper');
    ...
    var deleteOneProductConfirm = function () {
        $('[id*=delete_product]').on('click', function (e) {
            e.preventDefault();
    ...
    var deleteProductsConfirm = function () {
        $('#del_product_form').submit(function (e) {
            e.preventDefault();
            currentForm = this;
            bootbox.setDefaults({
                locale: "hu",
            ...
            bootbox.confirm("Biztosan törölni akarja a kijelölt termékeket?", function (result) {
                if (result) {
                    ...
                    //$('#del_product_form').append($("<input>").attr("type", "hidden").attr("name", "delete_product_submit").val("submit"));
                    currentForm.submit();
                }
            });
        });
    ...
    var enableDisableButtons = function () {
        ...
        var deleteProductSubmit = $('button[name="delete_product_submit"]');
        var checkAll = $('input.group-checkable');
        var checkboxes = $('input.checkboxes');

        deleteProductSubmit.attr('disabled', true);

        checkboxes.change(function () {
            $(this).closest("tr").find('.btn-group a').attr('disabled', $(this).is(':checked'));
            deleteProductSubmit.attr('disabled', !checkboxes.is(':checked'));
        });
    ...
    var makeActiveConfirm = function () {
        $('[id*=make_active], [id*=make_inactive]').on('click', function (e) {
            ...
            bootbox.confirm("Biztosan módosítani akarja a termék státuszát?", function (result) {
                if (result) {
                    makeActive(productId, action, url, elem);
                }
            });
        });
    }

    var makeActive = function (productId, action, url, elem) {
        //console.log(elem);
        $.ajax({
            type: "POST",
            data: {
            ...
            success: function (result) {
                if (result.status == 'success') {
                    ...
                    if (action == 'make_inactive') {
                        ...
                        //$(elem).attr('href', 'admin/products/make_active');
                        ...
                        //$(elem).attr('href', 'admin/products/make_inactive');
    ...
    var printTable = function () {
        $('#print_products').on('click', function (e) {
            e.preventDefault();
            var divToPrint = document.getElementById("products");
            console.log(divToPrint);
//		divToPrint = $('#products tr').find('th:last, td:last').remove();
            newWin = window.open("");
