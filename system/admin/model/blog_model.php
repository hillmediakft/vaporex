...
class blog_model extends Model {
    ...
    public function insert() {
        ...
        $data['blog_slug'] = Util::string_to_slug($_POST['blog_title']);
    ...
    public function delete_category($id) {
        ...
        if ($this->get_no_of_blogs_in_category($id) > 0) {
        ...
        if ($result == 1) {
            Message::set('success', 'Blog kategória sikeresen törölve!');
            return true;
        } else {
    ...
    /**
     * 	Lekérdezi a kategóriában lévő blogbejegyzések számát
     */
    public function get_no_of_blogs_in_category($id = null) {
        $this->query->reset();
        $result = $this->query->query_sql('SELECT blog_category, COUNT(*) FROM blog GROUP BY blog_category');

        $array = array();
        foreach ($result as $value) {
            foreach ($value as $value2) {
                $array[$value['blog_category']] = $value['COUNT(*)'];
            }
        }
        if (!isset($id)) {
            return $array;
