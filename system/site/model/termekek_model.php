...
class Termekek_model extends Site_model {
    ...
    public function product_categories($id = 0) {
        ...
        if (!empty($product_categories)) {
            ...
            foreach ($product_categories as $key => $value) {
                ...
                $html .= '<img src="' . $value['product_category_photo'] . '" alt="">';
                $html .= '</a>';
                $html .= '</div>';

                $html .= '<div class="product-bottom">';
                $html .= '<a href="' . $this->registry->site_url . 'termekek/' . 'kategoria/' . Util::string_to_slug($value['product_category_name']) . '/' . $value['product_category_id'] . '">' . $value['product_category_name'] . ' (' . $this->get_product_count($value['product_category_id']) . ')</a>';
                $html .= '</div>';
                $html .= '</div>';
                $html .= '</li>';
            }
    ...
    public function products_in_category($cat_id) {
        ...
        foreach ($products as $key => $value) {
            ...
            $html .= '<div class="product-container">';
            $html .= '<div class="product-image">';
            $html .= '<img src="' . $value['product_photo'] . '" alt="">';
            $html .= '</a>';
            $html .= '</div>';
...
// *************** ALKATEGÓRIA CIKLUS ***************** //
            foreach ($sub_categories as $sub_value) {
                ...
                foreach ($sub_sub_categories as $sub_sub_value) {
                    ...
                    foreach ($sub_sub_sub_categories as $sub_sub_sub_value) {
                        ...
                        foreach ($sub_sub_sub_products as $sub_sub_sub_product_value) {
                            ...
                            $list .= '<li><a href="' . $this->registry->site_url . 'termekek/' . $sub_sub_sub_product_value['product_id'] . '">' . $sub_sub_sub_product_value['product_title'] . '</a></li>';
                        }
                        $list .= '</ul>';
                        $list .= '</li>';
                    }
                    ...
                    foreach ($sub_sub_products as $sub_sub_product_value) {
                        ...
                        $list .= '<li><a href="' . $this->registry->site_url . 'termekek/' . $sub_sub_product_value['product_id'] . '">' . $sub_sub_product_value['product_title'] . '</a></li>';
                    }
                    $list .= '</ul>';
                    $list .= '</li>';
                }
                ...
                foreach ($sub_products as $sub_product_value) {
                    ...
                    $list .= '<li><a href="' . $this->registry->site_url . 'termekek/' . $sub_product_value['product_id'] . '">' . $sub_product_value['product_title'] . '</a></li>';
                }
                $list .= '</ul>';
                $list .= '</li>';
            }
            ...
            foreach ($products as $product_value) {
                ...
                $list .= '<li><a href="' . $this->registry->site_url . 'termekek/' . $product_value['product_id'] . '">' . $product_value['product_title'] . '</a></li>';
            }
...
//        $this->query->debug(true);
        ...
        $path = '<a href="' . $this->registry->site_url . 'termekek">Termékek</a> / ';
        foreach ($result as $value) {
            $path .= '<a href="' . $this->registry->site_url . 'termekek/' . 'kategoria/' . Util::string_to_slug($value['lev1']) . '/' . $value['id1'] . '">' . $value['lev1'] . '</a> / ';
        }
        return $path;
    }
